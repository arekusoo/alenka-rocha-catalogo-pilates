...
export const AgendaView: React.FC<AgendaViewProps> = ({
  ...
  // Counts for tabs
  const { upcomingCount, historyCount } = useMemo(() => {
    let upcoming = 0;
    let history = 0;
    sessions.forEach((s) => {
      if (s.status === 'completed' || s.date < todayStr) {
        history++;
      } else {
        upcoming++;
      }
    });
    return { upcomingCount: upcoming, historyCount: history };
  }, [sessions, todayStr]);
